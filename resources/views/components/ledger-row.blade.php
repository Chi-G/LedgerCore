...
<div class="grid grid-cols-[10px_1fr_90px] md:grid-cols-[14px_86px_1fr_130px_150px] items-center gap-3 md:gap-4 px-5 md:px-7 py-4 border-b border-[#EFEBE0] last:border-none hover:bg-[#FBFAF6] transition-colors">
    <span class="w-1.5 h-1.5 rounded-full {{ $isCredit ? 'bg-ledger-green' : 'bg-ledger-rust' }}"></span>

    <span class="hidden md:inline font-mono text-[11.5px] text-[#8B8474]">{{ $entry->reference }}</span>
    ...
    <span class="hidden md:inline font-mono text-[11.5px] text-[#9C9486]">
        {{ $entry->created_at->format('d M, H:i') }}
    </span>

    <span class="font-mono text-sm text-right font-medium {{ $isCredit ? 'text-ledger-green' : 'text-ledger-rust' }}">
